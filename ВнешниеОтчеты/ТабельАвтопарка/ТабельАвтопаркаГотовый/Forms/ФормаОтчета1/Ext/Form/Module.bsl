...
// ЦСМ +++
&НаСервере
Перем ItobЭтоПервоеОбновлениеЭлементовНастроек;
// ЦСМ ---
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ЦСМ +++
	ItobЭтоПервоеОбновлениеЭлементовНастроек = Истина;
	// ЦСМ ---
...
Процедура Подключаемый_ЭлементНастройки_ПриИзменении(Элемент)
	...
	Если (уатОбщегоНазначенияПовтИсп.ВариантПоставкиПРОФ()
		ИЛИ уатОбщегоНазначенияПовтИсп.ВариантПоставкиКОРП()
		ИЛИ уатОбщегоНазначенияПовтИсп.ВариантПоставкиТакси())
		И ЭтотОбъект.Элементы.Найти(ИмяЭлемента) = Элемент 
		И ЗаголовокЭлемента = Элемент.Заголовок Тогда
		МодульItobОтчетыКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ItobОтчетыКлиент");
		МодульItobОтчетыКлиент.ЭлементНастройкиПриИзменении(ЭтотОбъект, Элемент);
		ЭлементНастройкиПриИзмененииСервер(Элемент.Имя, Элемент.Заголовок);
	КонецЕсли;
	// ЦСМ ---
...
// + Профрешение Аун Д.В. 28.01.2026 - №3384
&НаСервере
...
&НаСервере
Функция ПолучитьОтчетТабличныйДокументОбработкаРасшифровкиНаСервере(Расшифровка)
	
	_ДанныеРасшифровки = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
	...
	Если ТабРасшифровки <> Неопределено Тогда
		...
		Возврат ТабРасшифровки;
	КонецЕсли;	
	
	Возврат Неопределено;
	
КонецФункции
// - Профрешение Аун Д.В. 28.01.2026
...
&НаКлиенте
Процедура Подключаемый_ПолеВвода_Регулирование(Элемент, Направление, СтандартнаяОбработка)
	
	Если уатОбщегоНазначенияПовтИсп.ВариантПоставкиПРОФ()
		ИЛИ уатОбщегоНазначенияПовтИсп.ВариантПоставкиКОРП()
		ИЛИ уатОбщегоНазначенияПовтИсп.ВариантПоставкиТакси() Тогда
		МодульItobОтчетыКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ItobОтчетыКлиент");
		МодульItobОтчетыКлиент.Регулирование(ЭтотОбъект, Элемент, Направление, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления)
	...
	Если уатОбщегоНазначенияПовтИсп.ВариантПоставкиПРОФ()
		ИЛИ уатОбщегоНазначенияПовтИсп.ВариантПоставкиКОРП()
		ИЛИ уатОбщегоНазначенияПовтИсп.ВариантПоставкиТакси() Тогда
		МодульItobОтчеты = ОбщегоНазначения.ОбщийМодуль("ItobОтчеты");
		МодульItobОтчеты.ПослеСозданияЭлементовНастроек(ЭтотОбъект, НастройкиОтчета);
	КонецЕсли;
	// ЦСМ ---
...
// ЦСМ +++
&НаСервере
Процедура ЭлементНастройкиПриИзмененииСервер(ИмяИзмененногоЭлемента, ЗаголовокИзмененногоЭлемента)
	
	МодульItobОтчеты = ОбщегоНазначения.ОбщийМодуль("ItobОтчеты");
	МодульItobОтчеты.ЭлементНастройкиПриИзмененииСервер(ЭтотОбъект, ИмяИзмененногоЭлемента, ЗаголовокИзмененногоЭлемента);
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	СтандартнаяОбработка = Ложь;
	ТабРасшифровки = ПолучитьОтчетТабличныйДокументОбработкаРасшифровкиНаСервере(Расшифровка);
