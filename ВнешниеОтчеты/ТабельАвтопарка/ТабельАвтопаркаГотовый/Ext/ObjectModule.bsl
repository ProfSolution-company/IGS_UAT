...
Функция ПолучитьТаблицуПростоев()
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	СУММА(прНевыходТехникиОбороты.КоличествоОборот) КАК Количество
	                |ИЗ
	                ...
	                |		ПО прНевыходТехникиОбороты.ТранспортноеСредство = уатТС.Ссылка
	                |
	                |СГРУППИРОВАТЬ ПО
	                |	прНевыходТехникиОбороты.ПричинаНевыхода,
	                |	прНевыходТехникиОбороты.Колонна,
	                |	прНевыходТехникиОбороты.ТранспортноеСредство,
	                |	прНевыходТехникиОбороты.Организация,
	                |	прНевыходТехникиОбороты.ПричинаНевыхода.Код,
	                |	ЕСТЬNULL(уатТС.Модель, ЗНАЧЕНИЕ(Справочник.уатМоделиТС.ПустаяСсылка)),
	                |	прНевыходТехникиОбороты.Период"; 			   
...
Функция ПолучитьТаблицуЭксплуатацииТранспортныхСредств(НачалоПериода, КонецПериода, ВключатьПрицепы = Ложь, БезСтатуса = Ложь, ПолучитьДоходыИЧасы = Ложь, МассивТС = Неопределено) 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РегистрСведений.уатСостояниеТС.СрезПоследних(&НачПериода, ) КАК уатСостояниеТССрезПоследних
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДанныеТС КАК ВТ_ДанныеТС
	               |		ПО уатСостояниеТССрезПоследних.ТС = ВТ_ДанныеТС.ОсновноеСредство
	               |ГДЕ
	               |	уатСостояниеТССрезПоследних.Состояние = ЗНАЧЕНИЕ(Справочник.уатСостояниеТС.СформированПутевойЛист)
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               ...
	               |	РегистрСведений.уатСостояниеТС КАК уатСостояниеТС
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДанныеТС КАК ВТ_ДанныеТС
	               |		ПО уатСостояниеТС.ТС = ВТ_ДанныеТС.ОсновноеСредство
	               |ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ ВключатьПрицепы И
			(Выборка.Модель.ТипТС.ВидТС = Перечисления.уатВидыТС.Прицеп ИЛИ
			Выборка.Модель.ТипТС.ВидТС = Перечисления.уатВидыТС.ПрицепСамосвал ИЛИ
			Выборка.Модель.ТипТС.ВидТС = Перечисления.уатВидыТС.ПрицепЦистерна ИЛИ
			Выборка.Модель.ТипТС.ВидТС = Перечисления.уатВидыТС.Полуприцеп) Тогда
			Продолжить;
	...
	Если ПолучитьДоходыИЧасы Тогда
		...
		Для каждого СтрокаТаблицаТСРегистратор Из ТаблицаТСРегистратор Цикл
			...
			//Если ТипЗнч(СтрокаТаблицаТСРегистратор.ПутевойЛист) = Тип("ДокументСсылка.уатПутевойЛист") Тогда
			//	_НачПериодаДокумент = СтрокаТаблицаТСРегистратор.ПутевойЛист.ДатаВыезда;
			//	_КонПериодаДокумент = СтрокаТаблицаТСРегистратор.ПутевойЛист.ДатаВозвращения;
			//ИначеЕсли ТипЗнч(СтрокаТаблицаТСРегистратор.ПутевойЛист) = Тип("ДокументСсылка.пт_АктОКонсервацииТехники") Тогда
			//	_НачПериодаДокумент = СтрокаТаблицаТСРегистратор.ПутевойЛист.ДатаНачала;
			//	_КонПериодаДокумент = СтрокаТаблицаТСРегистратор.ПутевойЛист.ДатаОкончания;
			//Иначе
			//	Продолжить;
			//КонецЕсли;
...
Функция ПолучитьПриоритетДляПростоя(ПричинаНевыхода)
	...
	Результат = 0;
	Если ПричинаНевыхода.Код = "000000008" Тогда // реализация
		Результат = 10;
	ИначеЕсли ПричинаНевыхода.Код = "000000009" Тогда // списание
		Результат = 9;
	ИначеЕсли ПричинаНевыхода.Код = "000000003" Тогда // простой сезонной техники
		Результат = 8;
	ИначеЕсли ПричинаНевыхода.Код = "000000004" Тогда // ожидание ремонта
		Результат = 7;
	ИначеЕсли ПричинаНевыхода.Код = "000000002" Тогда // ремонт
		Результат = 6;
	ИначеЕсли ПричинаНевыхода.Код = "000000007" Тогда // отсутствие водителя
		Результат = 5;
	ИначеЕсли ПричинаНевыхода.Код = "000000001" Тогда // отсутствие работы
		Результат = 4;
	КонецЕсли;
