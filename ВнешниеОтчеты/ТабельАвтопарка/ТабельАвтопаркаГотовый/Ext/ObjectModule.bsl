...
Функция ПолучитьТаблицуПростоев()
	...
	Запрос.Текст =  "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                ...
	                |	прПричиныНевыходаТС.Код КАК Код
	                |ПОМЕСТИТЬ ВТ_НевыходТехникиСКодом
	                |ИЗ
	                |	ВТ_НевыходТехники КАК ВТ_НевыходТехники
	                |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.прПричиныНевыходаТС КАК прПричиныНевыходаТС
	                |		ПО ВТ_НевыходТехники.ПричинаНевыхода = прПричиныНевыходаТС.Ссылка
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                |	ЕСТЬNULL(уатТС.Модель, ЗНАЧЕНИЕ(Справочник.уатМоделиТС.ПустаяСсылка)) КАК Модель,
	                |	ВТ_НевыходТехникиСКодом.Дата КАК Дата,
	                |	ВТ_НевыходТехникиСКодом.Организация КАК Организация,
	                |	ВТ_НевыходТехникиСКодом.Колонна КАК Колонна,
	                |	ВТ_НевыходТехникиСКодом.ТранспортноеСредство КАК ТранспортноеСредство,
	                |	ВТ_НевыходТехникиСКодом.ПричинаНевыхода КАК ПричинаНевыхода,
	                |	ВТ_НевыходТехникиСКодом.Количество КАК Количество,
	                |	ВТ_НевыходТехникиСКодом.Код КАК Код
	                |ИЗ
	                |	ВТ_НевыходТехникиСКодом КАК ВТ_НевыходТехникиСКодом
	                |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.уатТС КАК уатТС
	                |		ПО ВТ_НевыходТехникиСКодом.ТранспортноеСредство = уатТС.Ссылка"; 			   
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |ИЗ
	               |	РегистрСведений.уатСостояниеТС.СрезПоследних(&КонПериода, Состояние В (&СостоянияРемонта)) КАК уатСостояниеТССрезПоследних
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидТС = Перечисления.уатВидыТС.Прицеп ИЛИ
		...
		Пока ТекДата < Выборка.ОкончаниеРаботы Цикл
			Если ТекДата >= НачалоДня(НачПериода) И
				...
				Если НетРегистрацииРемонта Тогда
					...
					СтрокаТаблицаПростоев.Код = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочники.прПричиныНевыходаТС.Ремонт, "Код");
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ПараметрПериодОтчета = Компоновщикнастроек.Настройки.ПараметрыДанных.Элементы.Найти(Новый ПараметрКомпоновкиДанных("ПериодОтчета")).Параметр;
   	ПараметрОрганизация = Компоновщикнастроек.Настройки.ПараметрыДанных.Элементы.Найти(Новый ПараметрКомпоновкиДанных("Организация")).Параметр;
	
	НачПериода = Дата('00010101');
	...
	Для каждого элементПараметр Из Компоновщикнастроек.ПользовательскиеНастройки.Элементы Цикл
		...
		Если элементПараметр.Параметр = ПараметрПериодОтчета И элементПараметр.Использование Тогда
			НачПериода = элементПараметр.Значение.ДатаНачала;
			КонПериода = элементПараметр.Значение.ДатаОкончания;
		ИначеЕсли элементПараметр.Параметр = ПараметрОрганизация И элементПараметр.Использование Тогда
			Организация = элементПараметр.Значение;
		КонецЕсли;
...
Функция ПолучитьТаблицуВладенияТранспортныхСредств(НачалоПериода, КонецПериода, ВключатьПрицепы = Ложь, МассивТС = Неопределено) Экспорт 
	...
	Запрос.УстановитьПараметр("ПоВсемТС", ЗначениеЗаполнено(МассивТС));
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ВТ_МестонахождениеТС.ТС КАК ТранспортноеСредство,
	               |	ВТ_ДанныеТС.Модель КАК Модель,
	               |	ВТ_МестонахождениеТС.Организация КАК Организация,
	               |	ВТ_МестонахождениеТС.Колонна КАК Колонна,
	               ...
	               |		ПО ВТ_ДанныеТС.ОсновноеСредство = ВТ_МестонахождениеТС.ТС
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.уатМоделиТС КАК уатМоделиТС
	               |			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.уатТипыТС КАК уатТипыТС
	               |			ПО уатМоделиТС.ТипТС = уатТипыТС.Ссылка
	               |		ПО ВТ_ДанныеТС.Модель = уатМоделиТС.Ссылка
	...
	Пока ВыборкаПоТранспортнымСредствам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НЕ ВключатьПрицепы И
				(ВидТС = Перечисления.уатВидыТС.Прицеп ИЛИ
				ВидТС = Перечисления.уатВидыТС.ПрицепСамосвал ИЛИ
				ВидТС = Перечисления.уатВидыТС.ПрицепЦистерна ИЛИ
				ВидТС = Перечисления.уатВидыТС.Полуприцеп) Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ТекПериод <> Неопределено И
				...
				ЗаполнитьПериодВладенияТранспортногоСредства(Результат, ВыборкаПоТранспортнымСредствам.ТранспортноеСредство, ВидТС,  ТекОрганизация, ТекКолонна, ТекПериод, Выборка.Период - 86400, ДатаВводаВЭксплуатацию, ДатаВыбытия);
		...
		Если ТекПериод <> Неопределено Тогда
			Если НЕ ВключатьПрицепы И
				...
				ВидТС = Перечисления.уатВидыТС.Полуприцеп) Тогда
			Иначе
				ЗаполнитьПериодВладенияТранспортногоСредства(Результат, ВыборкаПоТранспортнымСредствам.ТранспортноеСредство, ВидТС, ТекОрганизация, ТекКолонна, ТекПериод, КонецПериода, ДатаВводаВЭксплуатацию, ДатаВыбытия);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТаблицуПересекающихсяПутевыхЛистов(НачалоПериода, КонецПериода)
    ...
    Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
                   ...
                   |
                   |ВЫБРАТЬ
                   |	ВТ_ПериодыРаботыТС.ТранспортноеСредство,
                   |	ВТ_ПериодыРаботыТС.Регистратор,
                   |	ВТ_ПериодыРаботыТССравнение.Регистратор
                   |ИЗ
                   |	ВТ_ПериодыРаботыТС_Предварительно КАК ВТ_ПериодыРаботыТС
                   |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПериодыРаботыТС_Предварительно КАК ВТ_ПериодыРаботыТССравнение
                   |		ПО ВТ_ПериодыРаботыТС.ТранспортноеСредство = ВТ_ПериодыРаботыТССравнение.ТранспортноеСредство
                   ...
                   |			И ВТ_ПериодыРаботыТС.ОкончаниеРаботы > ВТ_ПериодыРаботыТССравнение.НачалоРаботы
                   |			И ВТ_ПериодыРаботыТС.НачалоРаботы < ВТ_ПериодыРаботыТССравнение.ОкончаниеРаботы
                   |			И (ВТ_ПериодыРаботыТССравнение.Регистратор ССЫЛКА Документ.прАктОКонсервацииТехники)
                   |
                   |ОБЪЕДИНИТЬ ВСЕ
                   |
                   |ВЫБРАТЬ
                   ...
                   |			И ВТ_ПериодыРаботыТС.ОкончаниеРаботы > ВТ_ПериодыРаботыТССравнение.НачалоРаботы
                   |			И ВТ_ПериодыРаботыТС.НачалоРаботы < ВТ_ПериодыРаботыТССравнение.ОкончаниеРаботы
                   |			И (ВТ_ПериодыРаботыТС.Регистратор ССЫЛКА Документ.уатПутевойЛист
...
Функция ПолучитьТаблицуЭксплуатацииТранспортныхСредств(НачалоПериода, КонецПериода, ВключатьПрицепы = Ложь, БезСтатуса = Ложь, ПолучитьДоходыИЧасы = Ложь, МассивТС = Неопределено) 
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |ПОМЕСТИТЬ ВТ_СостоянияТС
	               |ИЗ
	               |	РегистрСведений.уатСостояниеТС.СрезПоследних(&НачПериода, Состояние = ЗНАЧЕНИЕ(Справочник.уатСостояниеТС.СформированПутевойЛист)) КАК уатСостояниеТССрезПоследних
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
...
Функция ПолучитьРасшифровкуПоДокументам(СтруктураЗначений) Экспорт
	ТаблицаДокументов = Новый ТаблицаЗначений;
	ТаблицаДокументов.Колонки.Добавить("Документ");
	ТаблицаДокументов.Колонки.Добавить("Отображение");
	
	НачалоПериода = НачалоДня(СтруктураЗначений.ДатаДетализации);
	КонецПериода = КонецДня(СтруктураЗначений.ДатаДетализации);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НачПериода", ДобавитьМесяц(НачалоПериода, -1));
	Запрос.УстановитьПараметр("КонПериода", КонецПериода + 1);
	Запрос.УстановитьПараметр("ТС", СтруктураЗначений.ТранспортноеСредство);
	...
	|	уатСостояниеТССрезПоследних.Регистратор КАК Регистратор
	|ИЗ
	|	РегистрСведений.уатСостояниеТС.СрезПоследних(&НачПериода, ТС = &ТС) КАК уатСостояниеТССрезПоследних
	|ГДЕ
	|	(уатСостояниеТССрезПоследних.Состояние = ЗНАЧЕНИЕ(Справочник.уатСостояниеТС.СформированПутевойЛист)
	|			ИЛИ уатСостояниеТССрезПоследних.Состояние В (&СостоянияРемонта))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И (уатСостояниеТС.Состояние = ЗНАЧЕНИЕ(Справочник.уатСостояниеТС.СформированПутевойЛист)
	|			ИЛИ уатСостояниеТС.Состояние В (&СостоянияРемонта))
	|	И уатСостояниеТС.Период МЕЖДУ &НачПериода И &КонПериода
	|
	|УПОРЯДОЧИТЬ ПО
	|	НачалоРаботы
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ТекДата < Выборка.ОкончаниеРаботы Цикл
			Если ТекДата >= НачалоПериода И
				...
				СтрокаТаблицаДокументов.Документ = Выборка.Регистратор;
				//СтрокаТаблицаДокументов.Отображение = ?(ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.пт_АктОКонсервацииТехники"), "К", "Н");
				Если ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.прАктОКонсервацииТехники") Тогда
					СтрокаТаблицаДокументов.Отображение = "К";
				ИначеЕсли ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.уатПутевойЛист") Тогда
					СтрокаТаблицаДокументов.Отображение = "Н";
				ИначеЕсли ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.уатРемонтныйЛист") Тогда
					СтрокаТаблицаДокументов.Отображение = "Р";
				Иначе
					СтрокаТаблицаДокументов.Отображение = "";
				КонецЕсли;
			КонецЕсли;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	пт_НевыходТехникиОбороты.Регистратор КАК Регистратор,
	|	пт_НевыходТехникиОбороты.ПричинаНевыхода КАК ПричинаНевыхода
	|ИЗ
	|	РегистрНакопления.прНевыходТехники.Обороты(&НачПериода, &КонПериода, Регистратор, ТранспортноеСредство = &ТС) КАК пт_НевыходТехникиОбороты";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтрокаТаблицаДокументов = ТаблицаДокументов.Добавить();
		СтрокаТаблицаДокументов.Документ = Выборка.Регистратор;
		СтрокаТаблицаДокументов.Отображение = Выборка.ПричинаНевыхода.НаименованиеСокращенное;
	КонецЦикла;
	
	ТаблицаДокументов.Свернуть("Документ, Отображение");
	
	Таб = Новый ТабличныйДокумент;
	Макет = ПолучитьМакет("МакетРасшифровки");
	Область = Макет.ПолучитьОбласть("Шапка");
	Область.Параметры.ТранспортноеСредство = СтруктураЗначений.ТранспортноеСредство;
	Область.Параметры.Период = ПредставлениеПериода(НачалоПериода, КонецПериода);
	Таб.Вывести(Область);
	
	Область = Макет.ПолучитьОбласть("Строка");
	Для каждого СтрокаТаблицаДокументов Из ТаблицаДокументов Цикл
		Область.Параметры.Заполнить(СтрокаТаблицаДокументов);
		Область.Параметры.Н = ТаблицаДокументов.Индекс(СтрокаТаблицаДокументов) + 1;
		Таб.Вывести(Область);
	КонецЦикла;
