...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ООО ПрофРешение Дьяконов В.А. 10.10.2025 №00-2523
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Смена2", "Видимость", Объект.ДвеСмены);  
	Объект.Дата = Новый СтандартныйПериод(ВариантСтандартногоПериода.Сегодня);
	// ООО ПрофРешение Дьяконов В.А. 10.10.2025 №00-2523
...
// ООО ПрофРешение Дьяконов В.А. 10.10.2025 №00-2523
&НаСервере
Процедура ЗаполнитьТаблицуТС(ЗаказНаТС)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	уатЗаказГрузоотправителяпрТС.ТранспортноеСредство КАК ТС
	|ИЗ
	|	Документ.уатЗаказГрузоотправителя.прТС КАК уатЗаказГрузоотправителяпрТС
	|ГДЕ
	|	уатЗаказГрузоотправителяпрТС.Ссылка = &ЗаказНаТС";
...
Процедура ЗаказНаТСПриИзменении(Элемент)
	...
	ЗаполнитьТаблицуТС(Объект.ЗаказНаТС);

КонецПроцедуры

&НаКлиенте
Процедура ДвеСменыПриИзменении(Элемент)
	
	Если Объект.ДвеСмены Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Смена2", "Видимость", Объект.ДвеСмены);
		Элементы.Смена.Заголовок = "Смена 1";
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Смена2", "Видимость", Объект.ДвеСмены);
		Элементы.Смена.Заголовок = "Смена";
	КонецЕсли;
...
// ООО ПрофРешение Дьяконов В.А. 10.10.2025 №00-2523
&НаКлиенте
Процедура ЗаполнитьСписокТС(Команда)
...
Процедура Записать(Команда)
	...
	Если МассивЗанятыхТС.Количество() > 0 Тогда 
		Для Каждого ЗанятоеТС Из МассивЗанятыхТС Цикл
			ТекстНСТР = НСтр("en='';ru='Для ТС ""%1"" на указанные даты выезда-возврата уже созданы Путевые листы'");
...
Функция СтруктураСтроки(ТекСтрока)
	...
	СтруктураСтроки.Вставить("Смена");
...
Процедура СформироватьПодготовкуСервер()
	...
	ПараметрыОтбора.Вставить("ПризнакВыбора", Истина);
	ТранспортноеСредство = ТС.Выгрузить(ПараметрыОтбора, "ТС");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТранспортноеСредство", ТранспортноеСредство);
	Запрос.УстановитьПараметр("ЗаказНаТС", Объект.ЗаказНаТС);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	уатЗаказГрузоотправителяпрТС.ТранспортноеСредство КАК ТС,
	|	уатЗаказГрузоотправителяпрТС.МодельТС КАК МодельТС,
	|	уатЗаказГрузоотправителя.Контрагент КАК Контрагент,
	|	уатЗаказГрузоотправителя.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	уатЗаказГрузоотправителяпрТС.АдресОтправления КАК АдресУбытия1,
	|	уатЗаказГрузоотправителяпрТС.АдресПрибытия КАК АдресПрибытия,
	|	уатЗаказГрузоотправителяпрТС.ВремяОтправления КАК ВремяУбытия,
	|	уатЗаказГрузоотправителяпрТС.ВремяПрибытия КАК ВремяПрибытия
	|ИЗ
	|	Документ.уатЗаказГрузоотправителя.прТС КАК уатЗаказГрузоотправителяпрТС
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.уатЗаказГрузоотправителя КАК уатЗаказГрузоотправителя
	|		ПО уатЗаказГрузоотправителяпрТС.Ссылка = уатЗаказГрузоотправителя.Ссылка
	|ГДЕ
	|	уатЗаказГрузоотправителяпрТС.ТранспортноеСредство В(&ТранспортноеСредство)
	|	И уатЗаказГрузоотправителяпрТС.Ссылка = &ЗаказНаТС";
	Выборка = Запрос.Выполнить().Выбрать();
	ПериодСмен = ПолучитьВремяСмен(Объект.Смена, Объект.Смена2);
	ТекДата = Объект.Дата.ДатаНачала;
	Сч = 1;
	Пока Выборка.Следующий() Цикл
		Пока ТекДата <= Объект.Дата.ДатаОкончания Цикл
			Если Объект.ДвеСмены Тогда
				НоваяСтрока = Подготовка.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				НоваяСтрока.Водитель = Объект.Водитель;
				НоваяСтрока.Смена = Объект.Смена;
				НоваяСтрока.ВремяУбытия = СоединитьДатуИВремя(ТекДата, ПериодСмен[0].ВремяОкончания);
				НоваяСтрока.ВремяПрибытия = СоединитьДатуИВремя(ТекДата, ПериодСмен[0].ВремяНачала);
				...
				НоваяСтрока = Подготовка.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				НоваяСтрока.Водитель = Объект.Водитель2;
				НоваяСтрока.Смена = Объект.Смена2;
				Если ПериодСмен.Количество() > 1 Тогда
					НоваяСтрока.ВремяУбытия = СоединитьДатуИВремя(ТекДата, ПериодСмен[1].ВремяОкончания);
					НоваяСтрока.ВремяПрибытия = СоединитьДатуИВремя(ТекДата, ПериодСмен[1].ВремяНачала);
				Иначе
					НоваяСтрока.ВремяУбытия = ТекДата;
					НоваяСтрока.ВремяПрибытия = ТекДата;
				КонецЕсли;
				НоваяСтрока.Подразделение = Объект.Подразделение;
				НоваяСтрока.НомерСтроки = Сч;
				НоваяСтрока.ПризнакВыбора = Истина;
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				НоваяСтрока.Водитель = Объект.Водитель;
				НоваяСтрока.Водитель2 = Объект.Водитель2;
				НоваяСтрока.Смена = Объект.Смена;
				НоваяСтрока.ВремяУбытия = СоединитьДатуИВремя(ТекДата, ПериодСмен[0].ВремяОкончания);
				НоваяСтрока.ВремяПрибытия = СоединитьДатуИВремя(ТекДата, ПериодСмен[0].ВремяНачала);
				НоваяСтрока.Подразделение = Объект.Подразделение;
...
&НаСервере
Функция ПолучитьВремяСмен(Смена, Смена2)
	
	ТЗСмены = Новый ТаблицаЗначений;
	ТЗСмены.Колонки.Добавить("Смена", Новый ОписаниеТипов("СправочникСсылка.УатСмены"));
	НоваяСтрока = ТЗСмены.Добавить();
	НоваяСтрока.Смена = Смена;
	Если Смена2 <> Справочники.уатСмены.ПустаяСсылка() Тогда
		НоваяСтрока = ТЗСмены.Добавить();
		НоваяСтрока.Смена = Смена2;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТЗСмены", ТЗСмены);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	уатСменыПериодыСмены.ВремяНачала КАК ВремяНачала,
	|	уатСменыПериодыСмены.ВремяОкончания КАК ВремяОкончания
	|ИЗ
	|	Справочник.уатСмены.ПериодыСмены КАК уатСменыПериодыСмены
	|ГДЕ
	|	уатСменыПериодыСмены.Ссылка В(&ТЗСмены)";
...
Функция СформироватьПЛСервер(ТекСтрока, СообщениеПредыдущийПЛ = "")
	...
	Если докПЛ.Ссылка.Пустая() Тогда
		ТекстНСТР = НСтр("ru='Ошибка при записи в строке %1'");
...
Процедура ВидимостьПриКопированииСПредПЛ()
	Если НЕ Объект.ЗаполнятьПоДокументам ИЛИ Объект.ТипЗаполнения = 0 Тогда
		...
		Элементы.ПодготовкаДоговор.Видимость = НЕ Объект.КопироватьЗаданияСПредыдущегоПЛ;
