...
Процедура ЗакэшироватьНеобходимыеДанные(СсылкаНаОбъект)//Тут собираем все необходимые данные для дальнейшего использования{СсылкаНаОбъект}
	...
	|	уатПутевойЛист.ВыдатьКоличествоГорючего КАК ВыдатьКоличесытвоГорючего,
	...
	|	ВТПутевойЛист.ВыдатьКоличествоГорючего КАК ВыдатьКоличествоГорючего,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТВодитель1.Водитель1 КАК Водитель1,
	|	ВТВодитель1.Серия КАК Серия1,
	|	ВТВодитель1.Номер КАК Номер1,
	|	ВТВодитель1.ДатаВыдачи КАК ДатаВыдачи1,
	|	ВТВодитель1.Категория КАК Категория1,
	|	ВТВодитель2.Водитель2 КАК Водитель2,
	|	ВТВодитель2.Серия КАК Серия2,
	|	ВТВодитель2.Номер КАК Номер2,
	|	ВТВодитель2.ДатаВыдачи КАК ДатаВыдачи2,
	|	ВТВодитель2.Категория КАК Категория2
	|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтруктураДокументы.Вставить("Удостоверения", Выборка);
	КонецЦикла;;	
...
Процедура ЗаполнитьДанныеДляПечати(СсылкаНаОбъект)
	...
	ДанныеДляПечати.Вставить("НомерУдостоверенияВодителя", ?(ПустаяСтрока(ОбъектПечати.Удостоверения.Серия1), "", СокрЛП(ОбъектПечати.Удостоверения.Серия1) + " ") + ОбъектПечати.Удостоверения.Номер1 + " от " + Формат(ОбъектПечати.Удостоверения.ДатаВыдачи1, "ДФ=dd.MM.yy"));
	...
	Если ЗначениеЗаполнено(ОбъектПечати.ПутевойЛист.Водитель2) Тогда
		...
		ДанныеДляПечати.Вставить("НомерУдостоверенияВодителя", СокрЛП(ДанныеДляПечати.НомерУдостоверенияВодителя) + ", " + ?(ПустаяСтрока(ОбъектПечати.Удостоверения.Серия2), "", СокрЛП(ОбъектПечати.Удостоверения.Серия2) + " ") + ОбъектПечати.Удостоверения.Номер2 + " от " + Формат(ОбъектПечати.Удостоверения.ДатаВыдачи2, "ДФ=dd.MM.yy"));
...
Функция уатПечать_3(ИмяМакета, ФлагПечати="") Экспорт	
	...
	Если ФлагПечати <> 2 Тогда
		...
		Обл.Параметры.НомерУдостоверенияВодителя = ?(СокрЛП(ОбъектПечати.Удостоверения.Серия1)="","",СокрЛП(ОбъектПечати.Удостоверения.Серия1) + " ") + ОбъектПечати.Удостоверения.Номер1;
		Если ДанныеДляПечати.ДатаВыписки >= '20230901' Тогда
			Обл.Параметры.НомерУдостоверенияВодителя = Обл.Параметры.НомерУдостоверенияВодителя + " от " + Формат(ОбъектПечати.Удостоверения.ДатаВыдачи1, "ДФ=dd.MM.yy");
		КонецЕсли;
...
Функция уатПечать_6С(ИмяМакета, ФлагПечати="") Экспорт	
	...
	Если ФлагПечати <> 2 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектПечати.ПутевойЛист.Водитель2) Тогда	
			...
			+ ?(СокрЛП(ОбъектПечати.Удостоверения.Серия2)="","",СокрЛП(ОбъектПечати.Удостоверения.Серия2) + " ") + ОбъектПечати.Удостоверения.Номер2;
			Если ОбъектПечати.ПутевойЛист.ДатаВыписки >= '20230901' Тогда
				Обл.Параметры.НомерУдостоверенияВодителя = Обл.Параметры.НомерУдостоверенияВодителя + " от " + Формат(ОбъектПечати.Удостоверения.ДатаВыдачи2, "ДФ=dd.MM.yy");
			КонецЕсли;
		КонецЕсли;
...
Функция уатПрочитатьРеквизитыТС(ТС, Реквизит = "", ДатаСреза = Неопределено, СведенияЕсть = Истина, флСообщать = Ложь) Экспорт
	...
	Если ТипЗнч(ТС) = Тип("СправочникСсылка.уатТС") Тогда    
		Если Реквизит = "" Тогда
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.уатМестонахождениеТС.СрезПоследних(&Дата, ТС = &ОсновноеСредство) КАК уатМестонахождениеТССрезПоследних
			|		ПО уатТС.ОсновноеСредство = уатМестонахождениеТССрезПоследних.ТС
			|ГДЕ
			|	уатТС.ОсновноеСредство = &ОсновноеСредство";
		Иначе
			Запрос = Новый Запрос;
			Если (Реквизит = "Организация" или Реквизит = "Колонна") Тогда
				Запрос.Текст = 
				"ВЫБРАТЬ РАЗРЕШЕННЫЕ
				|	уатМестонахождениеТССрезПоследних.Организация КАК Организация,
				|	уатМестонахождениеТССрезПоследних.Колонна КАК Колонна
				|ИЗ
				|	РегистрСведений.уатМестонахождениеТС.СрезПоследних(&Дата, ТС = &ОсновноеСредство) КАК уатМестонахождениеТССрезПоследних";
			Иначе
				Запрос.Текст = 
		...
		Запрос.УстановитьПараметр("ОсновноеСредство", ТС);
		...
		Если ВыборкаДетали.Следующий() Тогда
			Если Реквизит = "" Тогда 
				...
				Если ВыборкаДетали.Подразделение = NULL Тогда
					СтруктураРеквизитовТС.Вставить("Подразделение", Справочники.ПодразделенияОрганизаций.ПустаяСсылка());
				Иначе
		...
		Иначе  
			...
			Если НЕ ТС.Пустая() И флСообщать Тогда
				ОбщегоНазначения.СообщитьПользователю("Транспортное средство <" + ТС + ">: отсутствуют сведения", СтатусСообщения.Внимание);
			КонецЕсли;
			СведенияЕсть = Ложь;
