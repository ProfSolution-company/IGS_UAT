...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Печатать весь путевой лист (ИГС)", "ВесьПутевойЛист", ВызовСерверногоМетода, Ложь, "ПечатьMXL"); // ПрофРешение Аун Д.В. - Задача №6242 20.02.2026	 
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Печатать весь путевой лист и маршрутный лист (ИГС)", "ВесьПутевойЛистИМаршрутныйЛист", ВызовСерверногоМетода, Ложь, "ПечатьMXL");  // ПрофРешение Аун Д.В. - Задача №6242 20.02.2026 
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Печатать лицевую сторону путевого листа (ИГС)", "ЛицеваяСторонаПутевогоЛиста", ВызовСерверногоМетода, Ложь, "ПечатьMXL");  // ПрофРешение Аун Д.В. - Задача №6242 20.02.2026 
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Печатать оборотную сторону путевого листа (ИГС)", "ОборотнаяСторонаПутевогоЛиста", ВызовСерверногоМетода, Ложь, "ПечатьMXL");  // ПрофРешение Аун Д.В. - Задача №6242 20.02.2026 
...
Процедура ЗаполнитьДанныеДляПечати(СсылкаНаОбъект)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = 
	// {{ ПрофРешение Аун Д.В. - Задача №6242 20.02.2026 - Добавил дату и номер заказа на ТС	
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	уатПутевойЛистЗадание.Контрагент КАК Контрагент,
	...
	|	уатПутевойЛистЗадание.ЗаказНаТС КАК ЗаказНаТС,
	|	ЕСТЬNULL(уатЗаказГрузоотправителя.Номер, """") КАК НомерЗаказНаТС,
	|	ЕСТЬNULL(уатЗаказГрузоотправителя.Дата, """") КАК ДатаЗаказНаТС
	|ПОМЕСТИТЬ ВТОснова
	|ИЗ
	|	Документ.уатПутевойЛист.Задание КАК уатПутевойЛистЗадание
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.уатЗаказГрузоотправителя КАК уатЗаказГрузоотправителя
	...
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК Куст,
	|	ВТОснова.ДатаЗаказНаТС КАК ДатаЗаказНаТС,
	|	ВТОснова.НомерЗаказНаТС КАК НомерЗаказНаТС
	|ИЗ
	|	ВТОснова КАК ВТОснова
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда		
			Если НЕ ВыборкаДетальныеЗаписи.ЗаказНаТС = Документы.уатЗаказГрузоотправителя.ПустаяСсылка() Тогда
				// {{ ПрофРешение Аун Д.В. - Задача №6242 20.02.2026	
				Заявка = "№ " + ВыборкаДетальныеЗаписи.НомерЗаказНаТС + ", " + Формат(ВыборкаДетальныеЗаписи.ДатаЗаказНаТС, "ДФ=dd.MM.yyyy");
				// }} ПрофРешение Аун Д.В. - Задача №6242
			КонецЕсли;		
