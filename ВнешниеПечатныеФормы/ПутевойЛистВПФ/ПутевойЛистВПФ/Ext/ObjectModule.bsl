...
Перем ДополнительныеДанныеВШапке;
Перем ЭтоИГСАвто;
...
Функция СформироватьТабличныйДокумент(МассивОбъектов, КоллекцияПечатныхФорм)
	...
	Для Каждого мОбъект из МассивОбъектов Цикл
		...
		Попытка
			...
			ПечатьЧекЛиста = Ложь;
			МаршрутныйЛист = Ложь;
			ДополнительныеДанныеВШапке = "Начальник БДД - Никитенко Сергей Владимирович – 8-922-466-19-77";
...
Процедура ЗаполнитьДанныеДляПечати(СсылкаНаОбъект)
	...
	Перем мВодительскоеУдСерия, мВодительскоеУдНомер, мВодительскоеУдКатегория, мВодительскоеУдДатаВыдачи;	
	...
	уатОбщегоНазначения.уатУдостоверениеВодителя(ОбъектПечати.ПутевойЛист.Водитель1, мВодительскоеУдСерия, мВодительскоеУдНомер, мВодительскоеУдКатегория, мВодительскоеУдДатаВыдачи);	
	ДанныеДляПечати.Вставить("НомерУдостоверенияВодителя", ?(ПустаяСтрока(мВодительскоеУдСерия), "", СокрЛП(мВодительскоеУдСерия) + " ") + мВодительскоеУдНомер + " от " + Формат(мВодительскоеУдДатаВыдачи, "ДФ=dd.MM.yy"));
	...
	Если ЗначениеЗаполнено(ОбъектПечати.ПутевойЛист.Водитель2) Тогда
		...
		уатОбщегоНазначения.уатУдостоверениеВодителя(ОбъектПечати.ПутевойЛист.Водитель2, мВодительскоеУдСерия, мВодительскоеУдНомер, мВодительскоеУдКатегория, мВодительскоеУдДатаВыдачи);	
		ДанныеДляПечати.Вставить("НомерУдостоверенияВодителя", СокрЛП(ДанныеДляПечати.НомерУдостоверенияВодителя) + ", " + ?(ПустаяСтрока(мВодительскоеУдСерия), "", СокрЛП(мВодительскоеУдСерия) + " ") + мВодительскоеУдНомер + " от " + Формат(мВодительскоеУдДатаВыдачи, "ДФ=dd.MM.yy"));
...
Функция уатПечать_3(ИмяМакета, ФлагПечати="") Экспорт
	...
	Если ФлагПечати <> 2 Тогда
		...
		уатОбщегоНазначения.уатУдостоверениеВодителя(ДанныеДляПечати.Водитель1, мВодительскоеУдСерия, мВодительскоеУдНомер, мВодительскоеУдКатегория, мВодительскоеУдДатаВыдачи);	
		Обл.Параметры.НомерУдостоверенияВодителя = ?(СокрЛП(мВодительскоеУдСерия)="","",СокрЛП(мВодительскоеУдСерия) + " ") + мВодительскоеУдНомер;
		Если ДанныеДляПечати.ДатаВыписки >= '20230901' Тогда
			Обл.Параметры.НомерУдостоверенияВодителя = Обл.Параметры.НомерУдостоверенияВодителя + " от " + Формат(мВодительскоеУдДатаВыдачи, "ДФ=dd.MM.yy");
		КонецЕсли;
...
Функция уатПечать_3Спец(ИмяМакета, ФлагПечати="") Экспорт
	...
	Если ФлагПечати <> 2 Тогда
		...
		|	уатВидыДДД.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.уатВидыДДД КАК уатВидыДДД
		|ГДЕ
		|	НЕ уатВидыДДД.ПометкаУдаления
		|	И уатВидыДДД.Наименование ПОДОБНО ""%удост%машинист%""";
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ВидУдостоверения = Выборка.Ссылка;
		Иначе
